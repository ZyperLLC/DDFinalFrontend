...
export default function Profile() {
  ...
  const handlePrevDraw = () => {
    setCurrentDraw(prev => ({
      number: prev.number > 1 ? prev.number - 1 : 1,
      date: new Date(prev.date.getTime() - 24 * 60 * 60 * 1000) // go back 1 day
    }));
  };
  ...
  const handleNextDraw = () => {
    setCurrentDraw(prev => ({
      ...
      date: new Date(prev.date.getTime() + 24 * 60 * 60 * 1000) // go forward 1 day
    }));
  };

  const handleJumpToRound = () => {
    const input = prompt("Enter draw number:");
    if (!input) return;
    const roundNum = parseInt(input, 10);
    if (!isNaN(roundNum) && roundNum > 0) {
      setCurrentDraw({
        number: roundNum,
        date: new Date() // could fetch actual date from backend if needed
      });
    }
  };
